...
As a result, exact matching of long nucleotide k-mers has been used for taxonomic classification and similarity detection between closely related genomes, including strain-level, species-level, and genus-level comparisons (often using k-mer lengths 51, 31, and 21, respectively (CTB NOTE: metapalette uses k=41, I think; not sure there's a good citation here)).
...
To make analyses at this scale tractable, we use FracMinHash sketching to randomly subsample all available k-mers, retaining approximately (\~1/1000 nucleotide k-mers) and \~1/200 amino acid k-mers (CTB: formatting??)) [@doi:10.1101/2022.01.11.475838].
DNA FracMinHash sketches have been shown to accurately subsample (?? CTB: rephrase 'accurately subsample') genome datasets [@doi:10.1101/2022.01.11.475838].
